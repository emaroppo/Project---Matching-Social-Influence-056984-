...
class JointMAUCBLearner(UCBLearner):
    def __init__(self, n_nodes, n_seeds, initial_confidence=np.inf):
        ...
        self.confidence = np.full(n_nodes, initial_confidence) # Fill initial confidence with a large finite number
        
    def pull_arms(self):
        upper_confidence_bound = self.empirical_means + self.confidence
        pulled_arms = []
        while len(pulled_arms) < self.n_seeds:
            max_confidence_arms = np.where(upper_confidence_bound == upper_confidence_bound.max())[0]
