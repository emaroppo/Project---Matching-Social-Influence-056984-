...
class EXP3ProbLearner(Learner):
    ...
    def get_probabilities(self):
        ...
        probs = (1 - self.gamma) * (self.weights / total_weight) + (self.gamma / (self.n_nodes**2))
        return probs
        ...
        for i in range(self.n_nodes):
            for j in range(self.n_nodes):
                if self.n_pulls[i][j] != 0:
                    estimated_reward = self.edge_rewards[i][j] / self.get_probabilities()[i][j]
                    self.weights[i][j] *= np.exp((self.gamma / (self.n_nodes**2)) * estimated_reward)
