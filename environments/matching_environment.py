...
class MatchingEnvironment(Environment):
    ...
    def round(self, pulled_arms):
        rewards = [self.reward_matrix[pulled_arm[1:]] for pulled_arm in pulled_arms[1:]]
    ...
    def opt(self, customer_classes, product_classes):
        ...
        expected_rewards = np.hstack(
            (expected_rewards, np.zeros((expected_rewards.shape[0], 1)))
        )
        expected_rewards = np.vstack(
            (expected_rewards, np.zeros((1, expected_rewards.shape[1])))
        )
...
class MatchingEnvironment2(Environment):
    def __init__(self, reward_parameters, constant_nodes=True):
        if constant_nodes:
            reward_matrix = np.random.normal(reward_parameters[0], reward_parameters[1])
        else:
