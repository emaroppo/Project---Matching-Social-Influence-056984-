...
def compute_metrics(model, env):
    ...
    std_inst_regret = np.sqrt(env.optimal_rewards[:,1]**2 + model.expected_rewards[:,1]**2)
    ...
    metrics['cumulative_reward'] = np.array([expected_cumulative_reward, std_cumulative_reward]).T
    ...
    metrics['cumulative_regret'] = np.array([expected_cumulative_regret, std_cumulative_regret]).T
