...
def generate_graph(n_nodes, edge_rate, min_prob=0.1, max_prob=0.2, n_phases=1):
    ...
    graph_probabilities = (
        np.random.uniform(min_prob, max_prob, (n_phases, n_nodes, n_nodes))
        * graph_structure_expanded
    )
    if n_phases == 1:
...
def generate_reward_parameters(
    n_customer_classes, n_product_classes, min_reward=10, max_reward=20
):
    # reward parameters
    means = np.random.uniform(
        min_reward, max_reward, (n_customer_classes, n_product_classes)
    )
